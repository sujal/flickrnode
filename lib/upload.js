...
Upload.prototype.async = function(path_to_photo, suggested_filename, optional_arguments, callback) {
	...
	fs.stat(path_to_photo, function(err, stats) {
		...
		self._request.listenResponse(http_request, null, function(err, ticket_xml) {
			//pull off the ticket id and pass it back to the caller
			ticket_id = ticket_xml.split("<ticketid>")[1].split("</ticketid>")[0];
			callback(err, ticket_id);
		});
